...
ny_ldas <- orderBy(~-mean,ddply(f[f$City =="new_york",], 
                                ...
                                summarise,nrow=length(nmi), mean=mean(nmi),sd=sd(nmi)))
...
ny_f$Topic <- ordered(paste("Community",ny_f$Topic),levels=paste("Community",0:19))

ny_map = get_map(source="stamen",
                 location=as.numeric(ny_f[ny_f$Topic=="Community 11",c("Longitude","Latitude")][1,]),
                 zoom=11)
...
g2 <- g2 + facet_wrap(~Constraint,scales="free",nrow=1) + xlab("Statistic Value") + ylab("Community")
...
p <- ggplot(ny_f[ny_f$Topic=="Community 3",],aes(bottomLevel)) + geom_bar()
...
nyt <- ny_f[ny_f$Topic=="Community 3",]
